...
const messages = {
  'zh-CN': {
    ...
    config: {
      tabModel: '模型配置', tabSensor: '传感器', tabFile: '文件白名单', tabMcp: 'MCP 服务', tabApp: '应用白名单', tabAcp: 'ACP 接入', tabDeploy: '部署',
      ...
      tipDeploy: '运行环境依赖一键部署：uv / Node.js / 沙盒镜像 / 嵌入模型',
      ...
      deployRestartHint: '安装完成后需重启程序才能生效',
      deployVpnHint: '部署过程需从网络下载（GitHub / HuggingFace），请保持网络 / VPN 稳定',
      deployItemUv: 'uv', deployItemUvDesc: 'Python 依赖管理器，传感器与 ACP 接入依赖它',
      deployItemNode: 'Node.js', deployItemNodeDesc: 'JavaScript 运行时，部分 MCP 服务与工具依赖它',
      deployItemSandbox: '沙盒环境', deployItemSandboxDesc: 'Docker 沙盒镜像（Bash 命令执行环境），多源自动拉取，会先检查并安装 Docker',
      deployItemEmbedding: '嵌入模型', deployItemEmbeddingDesc: '本地向量化模型（约 120MB），记忆检索依赖它',
      deployChipReady: '已就绪', deployChipRunning: '部署中', deployChipSuccessPending: '已安装 · 重启生效',
      deployDockerChip: 'Docker', deployImageChip: '镜像',
      deployAction: '一键部署', deployRedeploy: '重新部署', deployStarted: '部署已启动，进度见日志',
      deployViewLog: '部署日志', deployLogEmpty: '暂无日志',
      deployRegistryLabel: '镜像源', deployRegistryPlaceholder: '如 ghcr.nju.edu.cn',
      deployRegistrySave: '保存', deployRegistrySaved: '镜像源已保存，下次拉取生效',
      deployRegistryHint: '拉取沙盒镜像时优先使用该源；留空则自动依次尝试 ghcr.io / 公共镜像站 / Docker Hub',
      ...
      migrateHint: '确认后：embedding 等数据搬迁到新位置；agent_vm 不搬迁，新位置将创建全新空沙盒目录（旧数据留在旧盘）；当前沙盒容器将被销毁并在重启后按新路径重建（容器内安装的软件会丢失）。随后自动重启程序生效，期间请勿操作。',
      confirmMigrate: '确认切换并重启', migrating: '切换中…',
    ...
    chat: {
      ...
      ackAllCleanHint: '接受全部更改（清理所有备份）', newTerminal: '新建终端', minimizeKeepRunning: '最小化（保留后台运行）', goToDeploy: '前往部署', gotIt: '知道了',
  ...
  'en-US': {
    ...
    config: {
      tabModel: 'Model Config', tabSensor: 'Sensors', tabFile: 'File Whitelist', tabMcp: 'MCP Servers', tabApp: 'App Whitelist', tabAcp: 'ACP Access', tabDeploy: 'Deploy',
      ...
      tipDeploy: 'One-click setup of runtime dependencies: uv / Node.js / sandbox image / embedding model',
      ...
      deployRestartHint: 'Restart the app after installation for changes to take effect',
      deployVpnHint: 'Deployment downloads from the network (GitHub / HuggingFace) — keep your network / VPN stable',
      deployItemUv: 'uv', deployItemNode: 'Node.js', deployItemSandbox: 'Sandbox', deployItemEmbedding: 'Embedding Model',
      deployChipReady: 'Ready', deployChipRunning: 'Deploying', deployChipSuccessPending: 'Installed · restart to apply',
      deployDockerChip: 'Docker', deployImageChip: 'Image',
      deployAction: 'Deploy', deployRedeploy: 'Redeploy', deployStarted: 'Deployment started — see log for progress',
      deployViewLog: 'Deployment Log', deployLogEmpty: 'No logs yet',
      deployRegistryLabel: 'Image Registry', deployRegistryPlaceholder: 'e.g. ghcr.nju.edu.cn/purrpod',
      deployRegistrySave: 'Save', deployRegistrySaved: 'Registry saved — applies to next pull',
      deployRegistryHint: 'Tried first when pulling (defaults to official ghcr.io/purrpod); falls back to public mirrors / Docker Hub',
      ...
      migrateHint: 'On confirm: embedding data moves to the new location; agent_vm is NOT migrated — a fresh empty sandbox dir is created there (old data stays on the old disk); the current sandbox container will be destroyed and recreated on the new path after restart (software installed inside the container is lost). The app then restarts to apply. Do not operate during the process.',
      confirmMigrate: 'Switch & Restart', migrating: 'Switching…',
    ...
    chat: {
      ...
      noFileOpen: 'No file open', selectFileExplorer: 'Select a file from the explorer', newChat: 'NEW CHAT', config: 'CONFIG', saveAll: 'SAVE ALL', edit: 'EDIT', closeEdit: 'CLOSE', coreModel: 'Core Model', backgroundModel: 'Background Model', visionAdvisor: 'Vision Advisor', back: 'Back', switchSession: 'SWITCH', evolve: 'EVOLVE', alarms: 'ALARMS', sensors: 'SENSORS', addAlarm: 'ADD ALARM', mcpMenu: 'MCP', skillMenu: 'SKILL', cronMenu: 'CRON', soulMenu: 'SOUL', sensorMenu: 'SENSOR', refreshPage: 'Refresh page', selfUrlBlocked: "Cannot open the app's own URL in the built-in browser",
      ...
      ackAllCleanHint: 'Accept all changes (clean all backups)', newTerminal: 'New Terminal', minimizeKeepRunning: 'Minimize (keep running in background)', goToDeploy: 'Go to Deploy', gotIt: 'Got it',
