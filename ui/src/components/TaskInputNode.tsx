...
export default function TaskInputNode({ id, data, selected }: any) {
  ...
  // 当变量列表发生变化时，强制 React Flow 重新计算 Handle 的位置，否则新加的连线端点点不上去
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-2 border-b-2 border-ink/15 pb-3">
        <div className="w-5 h-5 bg-[#a3be8c] border-2 border-ink shadow-[2px_2px_0px_0px_rgba(26,26,26,1)] -rotate-6 shrink-0" style={sketchyShape1}></div>
        <div className="font-black text-xl tracking-wider truncate" style={{ fontFamily: '"Comic Sans MS", cursive' }}>
          GLOBAL INPUT
        </div>
      </div>
      <p className="text-xs font-bold text-ink/60 mb-4 border-b-2 border-dashed border-ink/10 pb-2">定义此工作流所需的外部参数</p>

      {/* 变量列表与输出 Handle */}
      <div className="flex flex-col gap-3">
        {(data.dynamic_inputs || []).map((item: any, idx: number) => (
          <div key={`var-${item.key}-${idx}`} className="relative flex items-center justify-between bg-ink/5 p-2 border-2 border-ink border-dashed" style={sketchyShape3}>
            <div className="flex items-center gap-2">
              <Variable size={16} strokeWidth={2.5} className="text-[#a3be8c]" />
            ...
            <Handle 
              ...
              id={item.key} 
              className="!bg-ink !w-4 !h-4 !border-2 !border-paper !-right-[32px] z-10 hover:!bg-[#a3be8c] hover:!scale-125 transition-transform" 
            />
          </div>
        ))}
      </div>
      ...
      <button 
        ...
        className="mt-4 w-full flex items-center justify-center gap-2 bg-cream border-2 border-ink py-2 text-sm font-black hover:bg-[#a3be8c] hover:text-ink transition-all shadow-[2px_2px_0px_0px_rgba(26,26,26,1)] active:shadow-none active:translate-y-0.5"
        style={sketchyShape1}
      >
        <Plus size={16} strokeWidth={3} /> ADD VARIABLE
      </button>
      ...
      {showModal && createPortal(
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-ink/40 backdrop-blur-sm p-4 pointer-events-auto" onClick={(e) => e.stopPropagation()}>
          <div style={sketchyShape3} className="bg-paper border-4 border-ink shadow-[12px_12px_0px_0px_rgba(26,26,26,1)] w-full max-w-sm p-8 relative rotate-1">
            <h3 className="text-2xl font-black mb-4 tracking-widest text-[#a3be8c]" style={{ fontFamily: '"Comic Sans MS", cursive' }}>NEW PARAMETER</h3>
            <p className="font-bold mb-4 opacity-80">输入一个新的全局变量名称：</p>
            <input 
              autoFocus
              value={newVar}
            ...
            <div className="flex gap-4">
              <button onClick={() => setShowModal(false)} style={sketchyShape1} className="flex-1 py-3 bg-cream text-ink border-4 border-ink font-black shadow-[4px_4px_0px_0px_rgba(26,26,26,1)] hover:bg-sand transition-all active:shadow-none active:translate-y-1">
                CANCEL
              </button>
              <button onClick={handleAdd} style={sketchyShape2} className="flex-1 py-3 bg-[#a3be8c] text-ink border-4 border-ink font-black shadow-[4px_4px_0px_0px_rgba(26,26,26,1)] hover:bg-[#8eb072] transition-all active:shadow-none active:translate-y-1">
                CONFIRM
              </button>
            </div>
          </div>
