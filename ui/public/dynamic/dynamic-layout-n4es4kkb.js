...
window.addEventListener('message', (event) => {
  if (event.data && event.data.type === 'UPDATE_TEXT') {
    if (event.data.messages && Array.isArray(event.data.messages)) {
      ...
      // 把文本拼成排版引擎需要的长字符串
      BODY_COPY = msgs.map(m => m.text).join('');
    } else if (event.data.text) {
      // 向后兼容：如果只收到文本，使用默认颜色
      BODY_COPY = event.data.text;
    }
    
    preparedBody = getPrepared(BODY_COPY, BODY_FONT);
    scheduleRender();
  }
});
// =======================================================
