...
class PurrMemoClient(metaclass=SingletonMeta):
    ...
    def search(self, query: str = "", filters: dict = None):
        ...
        print(f"🔍 [PurrMemo] search_tool.search_memory_api | query={query!r} | filters={filters}")
        result = self.search_tool.search_memory_api(query=query, filters=filters)
        print(f"🔍 [PurrMemo] search_memory_api 返回 | result长度={len(result) if result else 0}")
        return result
