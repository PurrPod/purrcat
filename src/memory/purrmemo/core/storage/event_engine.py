...
class EventEngine(metaclass=SingletonMeta):
    ...
    def _init_db(self):
        ...
        self.conn.row_factory = sqlite3.Row  # 【新增】让 fetchall 直接返回类似 dict 的对象
    ...
    def search_fts_bm25(self, query, start_time=None, end_time=None, limit=50):
        ...
        cursor.execute(sql, tuple(params))
        return [{"id": r[0], "data": {'event_id': r[0], 'content': r[1], 'timestamp': r[2], 'source': r[3]}, "score": abs(r[4])} 
    ...
    def cleanup_old_events(self, days_threshold=90):
        ...
        try:
            cursor = self.conn.cursor()
            cursor.execute(f"SELECT event_id FROM {self.table_name} WHERE timestamp < ?", (cutoff_date,))
            old_ids = [row[0] for row in cursor.fetchall()]
            
            if not old_ids:
                return 0
                
            cursor.execute(f"DELETE FROM {self.table_name}_fts WHERE event_id IN (SELECT event_id FROM {self.table_name} WHERE timestamp < ?)", (cutoff_date,))
            cursor.execute(f"DELETE FROM {self.table_name} WHERE timestamp < ?", (cutoff_date,))
            self.conn.commit()
            return len(old_ids)
        except Exception as e:
            print(f"清理陈旧事件失败: {e}")
