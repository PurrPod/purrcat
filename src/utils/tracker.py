...
class Tracker:
    def __init__(self, tracker_base_dir=None):
        # 使用固定路径
        self.tracker_base_dir = (
            Path(tracker_base_dir) if tracker_base_dir else Path(TRACKER_DIR)
        )
        self.date = time.strftime("%Y-%m-%d", time.localtime())
        self.month_str = (
            "-".join(self.date.split("-")[:2]) if self.date else None
        )  # 提取出 "2026-03"
        self._resolve_path()
    ...
    def _resolve_path(self, date_str=None):
        ...
        if len(parts) >= 2:
            ...
            self.daily_path = (
                self.tracker_base_dir / year_month / target_date / "daily.jsonl"
            )
