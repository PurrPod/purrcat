...
class Node(BaseNode):
    ...
    async def execute(self, inputs: Dict[str, Any], context: Any) -> Dict[str, Any]:
        ...
        self.log(context, "SYSTEM", f"📥 [消息构建] 收到输入包裹: {list(inputs.keys()) if inputs else '空'}")
        ...
        self.log(context, "SYSTEM", f"📝 [消息构建] 内容 (长度 {len(content)}): {content_preview}")
        
        message = {"role": role, "content": content}
        self.log(context, "SYSTEM", f"📤 [消息构建] 输出消息卡片: role={role}, content_length={len(content)}")
