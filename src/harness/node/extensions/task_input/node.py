...
class Node(BaseNode):
    ...
    async def execute(self, inputs: Dict[str, Any], context: Any) -> Dict[str, Any]:
        ...
        self.log(
            context,
            "SYSTEM",
            f"📋 [全局输入] 配置的全局变量: {[v.get('name') if isinstance(v, dict) else v for v in global_vars]}",
        )

        self.log(
            context,
            "SYSTEM",
            f"📥 [全局输入] 收到外部启动参数，键值: {list(context.inputs.keys())}",
        )
        ...
        self.log(
            context,
            "SYSTEM",
            f"📤 [全局输入] 暴露 {len(context.inputs)} 个变量给下游节点",
        )
