...
class Node(BaseNode):
    ...
    async def execute(self, inputs: Dict[str, Any], force_push_msgs: list, context: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            # 3. 核心：动态执行代码字符串。
            # 将执行后产生的函数、变量统统装进 local_scope 字典中
            exec(code_str, {}, local_scope)

            # 4. 校验用户是否按照规范写了 evaluate 函数
            ...
            # 6. 打包输出：把原始的 inputs 数据挂载到被选中的把手(Handle)上
            outputs = {
                "selected_branch": selected_branch,
                selected_branch: inputs  # 只有连在这个输出点上的下游节点，才能拿到非空数据
            }
            ...
            # 捕获详细的报错堆栈，对用户排查自己写的 Python 代码 bug 非常有帮助
