...
class Node(BaseNode):
    ...
    async def execute(self, inputs: Dict[str, Any], context: Any) -> Dict[str, Any]:
        ...
        if not os.path.isfile(target_path):
            ...
            raise IsADirectoryError(f"目标不能是文件夹，必须是具体的文件: {target_path}")
        ...
        try:
            ...
            self.log(context, "ERROR", f"❌ [本地读取] 尝试读取二进制文件 (如 PDF/图片) 失败")
            raise ValueError(f"读取失败：{target_path} 似乎是二进制文件（如 PDF、图片、压缩包），当前节点仅支持读取纯文本文件！")
        except PermissionError:
            self.log(context, "ERROR", f"❌ [本地读取] 权限不足，无法读取: {target_path}")
            raise PermissionError(f"权限不足：当前系统账户没有读取该文件的权限: {target_path}")
        except Exception as e:
            self.log(context, "ERROR", f"❌ [本地读取] 发生未知错误: {e}")
            raise RuntimeError(f"读取文件时发生系统级错误: {str(e)}")

        self.log(context, "SYSTEM", f"✅ [本地读取] 成功读取，内容长度: {len(content)} 字符")
