    ...
    async def execute(self, inputs: Dict[str, Any], context: Any) -> Dict[str, Any]:
        ...
        if isinstance(raw_data, str):
            try:
                ...
                self.log(context, "WARNING", "⚠️ [JSON提取] 传入的数据不是合法的 JSON 字符串，将按空字典处理")
        elif isinstance(raw_data, dict):
            data_dict = raw_data
        else:
            self.log(context, "WARNING", f"⚠️ [JSON提取] 不支持的数据类型: {type(raw_data)}，需要 dict 或 json 字符串")
        ...
        self.log(context, "SYSTEM", f"✅ [JSON提取] 完成，共提取了 {extracted_count} 个字段分配给下游")
