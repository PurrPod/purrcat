...
class Node(BaseNode):
    ...
    async def execute(self, inputs: Dict[str, Any], context: Any) -> Dict[str, Any]:
        self.log(
            context,
            "SYSTEM",
            f"📥 [JSON构建] 收到输入包裹: {list(inputs.keys()) if inputs else '空'}",
        )
        ...
        self.log(
            context,
            "SYSTEM",
            f"📋 [JSON构建] 配置的键值对列表: {[item.get('name') for item in kv_pairs if isinstance(item, dict) and item.get('name')]}",
        )
        ...
        for item in kv_pairs:
            ...
            self.log(
                context, "SYSTEM", f"  🔹 [{key_name}] = {val_preview} (来源: {source})"
            )
        ...
        try:
            ...
            self.log(
                context,
                "SYSTEM",
                f"📤 [JSON构建] 输出 JSON (长度 {len(json_string)}): {json_string[:200]}{'...' if len(json_string) > 200 else ''}",
            )
        except TypeError as e:
            self.log(
                context,
                "ERROR",
                f"❌ [JSON构建] 字典中包含无法序列化的对象，尝试强转字符串: {e}",
            )
