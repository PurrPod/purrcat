...
class Node(BaseNode):
    ...
    async def execute(self, inputs: Dict[str, Any], context: Any) -> Dict[str, Any]:
        ...
        self.log(
            context,
            "SYSTEM",
            f"📥 [模板渲染] 收到输入包裹: {list(inputs.keys()) if inputs else '空'}",
        )
        ...
        template_preview = (
            template_str[:150] + "..." if len(template_str) > 150 else template_str
        )
        ...
        self.log(
            context,
            "SYSTEM",
            f"🔑 [变量注入] 共 {len(variables)} 个变量: {list(variables.keys())}",
        )
        for var_name, var_val in variables.items():
            val_preview = (
                str(var_val)[:80] + "..." if len(str(var_val)) > 80 else str(var_val)
            )
        ...
        try:
            ...
            self.log(
                context,
                "SYSTEM",
                f"✅ [模板渲染] 渲染成功，输出长度: {len(rendered_content)}",
            )
            output_preview = (
                rendered_content[:200] + "..."
                if len(rendered_content) > 200
