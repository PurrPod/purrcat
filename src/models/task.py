...
2. **Web 工具的局限**：本地 `web` 工具仅适用于基础搜索和静态页面抓取。遇到强反爬虫限制或动态网页时，不要反复重试无效操作，应当使用 `search_in_system` 搜索指定插件的 MCP 工具查看有无对应的定制化工具
...
4. **利用现有技能**：每次开始任务之前，都要使用 `search_in_system` 搜索相关技能，查看是否有提供的技能对应上了任务需求，要用 load_skill 加载一下获取更专业的指导并获取脚本信息，这样能让你的交付物更加专业，执行更加高效！！！
"""
