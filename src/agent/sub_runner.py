...
class SubAgentRunner:
    ...
    def _save_history(self):
        SessionStore.save_session(
            self.main_session_id, self.messages, branch_id=self.internal_branch_id,
            deliverable=self.deliverable_path, action=self.action
        )
    ...
    async def run(self):
        ...
        while True:
            ...
            if not tool_calls:
                if file_exists:
                    self._notify_main(f"✅ [后台捷报] 子分支 `{self.display_branch_id}` 任务已圆满结束！目标交付物文件已就绪。")
                    break
                else:
                    self.messages.append({"role": "user", "content": f"任务未结束：未在沙盒中检测到要求的交付物文件 {self.deliverable_path}。"})
...
async def run_dag_graph(sub_branches: list, main_session_id: str, main_history: list):
    ...
    plan_batch_id = uuid.uuid4().hex[:6]  # 🌟 每次调用分配唯一批次号，防止 b1, b2 命名冲突
